		...
		if (is_null(self::$etape)) { // Liste des étapes
			...
			if (is_null($fonction)) {// Etape temporaire ou dimensions
				if (self::$etape == -1) {
                    ...
                    $options=$this->Modele_tranche->get_options_ec_v2(self::$pays, self::$magazine, self::$etape, self::$numero, true, true, $nom_option);
			}
			else {
				...
				if ($this->Modele_tranche->has_no_option(self::$pays, self::$magazine)) {
                    ...
                    $options=$this->Modele_tranche->get_options_ec_v2(self::$pays, self::$magazine, self::$etape, self::$numero, false, $nom_option);
				}
			}
