...
$no_database=true; // Ne pas utiliser les param�tres de connexion classiques
...
class Modele_tranche extends CI_Model {
	...
	function get_options($pays,$magazine,$ordre,$nom_fonction,$numero=null,$creation=false,$inclure_infos_options=false, $nouvelle_etape=false, $nom_option=null) {
		...
		$f=new $nom_fonction($resultats_options,false,$creation,!$nouvelle_etape); // Ajout des champs avec valeurs par d�faut
...
// 		echo $requete_get_options."\n";
	...
	function getValeur($option_nom,$option_valeur) {
		$texte='';
		if (!is_array($option_valeur)) // Valeur par d�faut
	...
	function get_liste($fonction,$type,$arg=null,$arg2=null) {
		...
		switch($type) {
			...
			case 'Photos': 
				...
				if (($dir = @opendir($rep)) === false) { // Sans doute un nouveau pays, on cr�e le sous-dossier
		...
		if ($creation) {
			...
			foreach(array_keys($champs) as $nom) {
				if (!isset($this->options->$nom) || (strpos('Couleur', $nom)!==false && $this->options->$nom==array())) {
					self::erreur('Le champ "'.$nom.'" est ind�fini !');
				}
			}
		}
	}
...
class Dimensions extends Fonction_executable {
	...
	function verifier_erreurs() {
		if ($this->options->Dimension_x < 0 || $this->options->Dimension_y < 0 ) {
			self::erreur('Dimensions n�gatives');
		}
...
class Image extends Fonction_executable {
	...
	function verifier_erreurs() {
		if (empty($this->options->Source)) {
			self::erreur('Le fichier n\'a pas �t� d�fini');
		}
		$chemin_reel=Image::get_chemin_reel($this->options->Source);
...
class TexteMyFonts extends Fonction_executable {
	...
	function TexteMyFonts($options,$executer=true,$creation=false,$get_options_defaut=true,$options_avancees=true) {
		...
		$post=new MyFonts($this->options->URL,
						  ...
						  $this->options->Chaine.'                                    .',
						  intval((Viewer_wizard::$largeur/1.5)/0.35) // Pr�cision
						 );
		$texte=$post->im;
		...
		if ($options_avancees) { // D�coupage des espaces blancs => Pas pour le preview
			...
			for ($i=0;$i<$width;$i+=2*$height) {
				...
				if (imagecolorstotal($image_decoupee) == 1) { // Image remplie uniform�ment => d�coupage
	...
	function verifier_erreurs() {
		if (is_array($this->options->Couleur_fond) && count($this->options->Couleur_fond) ==0)
			self::erreur('Couleur de fond ind�finie');
		if (is_array($this->options->Couleur_texte) && count($this->options->Couleur_texte) ==0)
			self::erreur('Couleur de texte ind�finie');
	}
}
