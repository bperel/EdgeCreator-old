...
deploy() {
  ...
  docker exec ${container_name} /bin/bash -c "mkdir -p ${webdir}/_sessions && cp -rp ${webdir}/_sessions ${webdir}_new" && \
  docker exec ${container_name} /bin/bash -c "rm -rf ${webdir} && mv ${webdir}_new ${webdir} && chmod 777 -R ${webdir}/_sessions" && \
