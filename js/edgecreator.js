	...
	$.each(['horizontale', 'verticale'], function (i, orientation_regle) {
		...
		image_regle.load(function () {
			...
			if ($(this).hasClass('horizontale')) {
				...
				div_regles.css({
					'marginTop': pos_premier_numero.top - decalage_regle,
					'marginLeft': pos_premier_numero.left - decalage_regle
				});
				positionsDepartRegles.push({'top': div_regles.css('marginTop'), 'left': div_regles.css('marginLeft')});

				$(this).css({
					'marginLeft': decalage_regle,
					'width': etendue,
...
function valider_modifier_valeur() {
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (recharger_etape) {
				charger_etape(etape_en_cours, nom_option, true);
			}
...
function adapter_scroll_reperes() {
	$.each($('.repere'), function (i, repere) {
		$(repere).css({
			'marginTop': ((-1) * $('#viewer_inner').scrollTop()) + 'px',
...
function charger_etape(num_etape, nom_option_sel, recharger) {
